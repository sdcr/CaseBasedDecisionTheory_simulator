...
/**
 * This class is a subclass of the {@link VirtualNodeContentVisitor} suited for
 * the computation with {@link BigDecimal} objects.
 * 
 * @author Stephan da Costa Ribeiro
 * 
 */
public class BigDecimalVirtualNodeContentVisitor extends
		VirtualNodeContentVisitor {
	...
	public BigDecimalVirtualNodeContentVisitor(Parameters parameters,
		...
		childContentGenerator = new BigDecimalChildNodeContentGenerator(
				outcomeMatrix, new BigDecimal(
	...
	protected void visitRecursively(int stage, int index)
		...
		if (leafStage != null && leafStage == stage)
			...
			BigDecimalMatrixNodeContent parentContent = bigDecimalSimState.contentsMatrix[stage][index];
			actionSelector.computeSelectedActions(selectedActionsIndices,
					parentContent);
			int numberOfSelectedActions = getNumberOfSelectedActions();

			BigDecimal multiActionProbability = big_one.divide(new BigDecimal(
					numberOfSelectedActions), mathContext);
			...
			for (int i = 0; i < numberOfSelectedActions; i++) {
				...
				for (int outcomeIndex = 0; outcomeIndex < outcomeMatrix[selectedActionIndex].length; outcomeIndex++) {
					BigDecimalMatrixNodeContent childContent = bigDecimalSimState.contentsMatrix[childrenStage][childIndex];
					...
					if (leafStage == null) {
						childrensExpectedUtilitySum = childrensExpectedUtilitySum
								.add(childContent.probabilityProduct
										.multiply(
						...
						if (commonConfig.isCalculateLowestAspirationLevels())
							updateLowestAspirationUtilities(
									childContent.aspirationLevel, childrenStage);
					}
					if (leafStage == null
							&& commonConfig
									.isCalculateRelativeActionOccurances()) {
						bigDecimalSimState.relativeActionOccurances[childrenStage][selectedActionIndex] = bigDecimalSimState.relativeActionOccurances[childrenStage][selectedActionIndex]
								.add(childContent.probabilityProduct,
										NodeVisitor.mathContext);
					}
			...
			if (leafStage == null)
				bigDecimalSimState.expectedUtilities[childrenStage] = bigDecimalSimState.expectedUtilities[childrenStage]
						.add(childrensExpectedUtilitySum,
								NodeVisitor.mathContext);
	...
	private void updateLowestAspirationUtilities(BigDecimal aspirationLevel,
			int stage) {
		if (bigDecimalSimState.lowestAspirationLevels[stage]
				.compareTo(aspirationLevel) == 1) {
