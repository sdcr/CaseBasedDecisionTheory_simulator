...
public class ParameterValidator {
	...
	public void checkValidity(Parameters parameters) throws InvalidActorActionException{
		if(parameters.getActorActions() == null || parameters.getActorActions().isEmpty())
			throw new InvalidActorActionException("There must be at least one actor action defined.");
		for(ActorAction action : parameters.getActorActions()){
			if (!hasValidProbabilityDistribution(action)) {
				throw new InvalidActorActionException("The probability distribution of the ActorAction "
						+action+" is invalid.");
			}
		}
	}
